...
class FasterWhisperASR(ASRBase):
    ...
    def transcribe(self, audio, init_prompt=""):
        ...
        elif isinstance(audio, np.ndarray) and audio.dtype != np.float32:
...
class HypothesisBuffer:
            ...
            if abs(a - self.last_commited_time) < 1:
                if self.commited_in_buffer:
                    ...
                    for i in range(
                        ...
                        tail = " ".join(
                            self.new[j - 1][2] for j in range(1, i + 1)
                        )
                        if c == tail:
                            words = []
                            for j in range(i):
    ...
    def pop_commited(self, time):
        while (
            self.commited_in_buffer and self.commited_in_buffer[0][1] <= time
        ):
...
class OnlineASRProcessor:
    ...
    def insert_audio_chunk(self, audio):
        """Insert audio chunk into the buffer.
    ...
    def process_iter(self):
        ...
        if (
            o and self.buffer_trimming_way == "sentence"
    ...
    def insert_audio_chunk(self, audio):
        ...
        if res is not None:
            ...
            if "start" in res and "end" not in res:
                ...
                self.online.init(
                    offset=(beg + self.buffer_offset) / self.SAMPLING_RATE
                )
                self.online.insert_audio_chunk(send_audio)
                self.current_online_chunk_buffer_size += len(send_audio)
                self.is_currently_final = True
...
def asr_factory(args, logfile=sys.stderr):
    ...
    if getattr(
        args, "vad", False
    ):  # Checks if VAD argument is present and True
...
if __name__ == "__main__":
    ...
    def output_transcript(o, now=None):
        ...
        if o[0] is not None:
            print(
                "%1.4f %1.0f %1.0f %s"
                % (now * 1000, o[0] * 1000, o[1] * 1000, o[2]),
                file=logfile,
                flush=True,
            )
            print(
